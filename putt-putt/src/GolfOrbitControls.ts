...
const golfOrbitControls = ecs.registerComponent({
  name: 'Golf Orbit Controls',
  schema: {
    speed: ecs.f32,
    ...
    // NEW: Starting camera orientation and distance
  ...
  schemaDefaults: {
    ...
    startYaw: 0,        // default facing "forwards"
  ...
  stateMachine: ({world, eid, schemaAttribute, dataAttribute}) => {
    ...
    const handleEnter = () => {
      const {
        ...
        controllerSupport,
        startYaw, startPitch, startDistance,  // <-- new schema values
      } = schemaAttribute.get(eid)
      ...
      // Set start values from schema (in degrees)
